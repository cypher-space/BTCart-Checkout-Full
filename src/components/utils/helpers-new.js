const host = "https://api.getalby.com/lnurl";
...
async function fetchInvoice(to, amount, comment) {
  const response = await fetch(host + `/generate-invoice?ln=${to}&amount=${amount}&comment=${comment}`, {
    headers: {
...
async function fetchParams(to) {
  const response = await fetch(host + `/lightning-address-details?ln=${to}`, {
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
    },
